...
$('.slide-navigation__txt span').on('click', function () {
    ...
    // Save the selected order in localStorage
...
$(document).ready(function () {
  ...
  var savedOrder = localStorage.getItem('selectedOrder');

  if (savedOrder) {
      // If there is a saved order, activate the slide for the saved order
      activate_slide(savedOrder);

      // Set the corresponding navigation item as active
      $('.slide-navigation__txt span[data-order="' + savedOrder + '"]').addClass('active');
  } else {
      // If no saved order, activate the first slide (default)
      activate_slide(1);
      $('.slide-navigation__txt span[data-order="1"]').addClass('active');
  }

  // Initial slide setup
  // Only do this if there's no saved order
  if (!savedOrder) {
